...
<img src=".github/banner-v2.png" alt="VLS â€” Velocity Context: reach 32 million tokens, execute a fraction of a percent" width="100%">
...
**[Install](#install)** &nbsp;Â·&nbsp; **[Velocity Context](#velocity-context)** &nbsp;Â·&nbsp; **[Everything in it](#everything-in-it)** &nbsp;Â·&nbsp; **[Mesh](#5-mesh--every-machine-in-your-house-as-one)** &nbsp;Â·&nbsp; **[Benchmarks](#measured-not-claimed)** &nbsp;Â·&nbsp; **[API](#connect-anything)** &nbsp;Â·&nbsp; **[FAQ](#questions-people-actually-ask)**
...
### A 4B model with an 8,192-token window answered across 32,000,000 tokens in 653 ms â€” on a 6 GB laptop card
...
|---|---|---|---|
| **cost per turn** | every token in the window, in VRAM, every time | an embedding call, then a database query | the passage only â€” **0.015%** of the context |
...
runs a passage â€” so **thirty-two times the material costs the same half second**, and the fraction
...
The installer looks at the machine before it downloads anything and takes the build that suits it â€”
...
thousand, a hundred and twenty-eight â€” whatever it is, the work you actually have is bigger than
...
what it read. The model's window never changes â€” it stops mattering.
...
from those windows to the ones they point at â€” a second hop, for questions whose answer sits beside
somebody the question never named â€” and assembles what fits. A single linear pass over token ids
...
A question does not read the context. It reaches into it â€” so what the model executes is a
...
were 92.0% at 128k and 86.0% at 1M, against <b>0.0%</b> for the same prompts with the text removed â€”
...
Every answer carries a receipt â€” not a log you go and find, but part of the response:
...
model failed to read it â€” and those two need opposite fixes.
...
VLS â€” editors, agents, scripts, your own code. Streaming, tool calls, the usual sampling knobs.

The **API** page shows the address of *this machine on your network*, not `localhost`, next to the
key and a request you can paste. It reports `context_length` as **the model's window** â€” the most
one request may contain â€” and reports how far the memory reaches, how much of it is used and what
...
the download starts â€” rather than after eight gigabytes have arrived.
...
Each conversation has its own memory, so two chats â€” or two people â€” do not overwrite each other.

### 4. Flows â€” work VLS does on its own
...
the answer, write the result to a file â€” on a schedule, without you. A wire carries text, the order
comes from the graph, and a loop is refused rather than run.

Everything runs on this machine. No accounts, no OAuth, no webhooks out â€” and that is a decision,
...
### 5. Mesh â€” every machine in your house as one
...
on and they **find each other by themselves** â€” no addresses to type, no configuration files. One
...
nothing until it is *joined*, and the main role can be handed from one machine to another â€” the new
...
prompt â€” measured on **your** machine with **your** model, kept as a record of runs that actually
...
and why it was chosen. Usage and client history survive restarts **and updates** â€” a record that
...
<sub>4kâ€“64k are five samples a cell; 128k is twenty â€” 260 runs in that column alone.</sub>
...
ever "too long to read" â€” what changes is how much material selection has to choose from. **The 16k
...
Past 128k NVIDIA's harness stops, so the ladder below is **our own** â€” same task definitions, our
haystack, our scorer â€” on the same machine and the same window. The two are reported separately and
...
<sub>Five samples a cell, 315 runs. <b>Nine of RULER's thirteen tasks</b> â€” niah_multikey_2,
...
|---|---|---|
| **Velocity Context** | Compiles your documents once, selects the passages a question needs, and hands the model only those â€” a fraction of a percent of the context per question. This is the technology; it is what makes a small window stop mattering. | **ours** |
...
| **Velocity / MTA** | Our own execution runtime for `.mfy` artifacts â€” the research line behind Velocity. Selected automatically for models built for it. | **ours** |
...
what it cost, and â€” with `velocity_excerpt` â€” the exact text the model was given.
...
update never touches them â€” updating VLS must never cost you a re-download of models. To remove it,
delete the program directory; your models and memories stay until you delete those too.

**Updating:** Settings â†’ Service says whether a newer build exists and installs it in place, with a
...
Yes â€” turn on network access and other machines on your network can reach the same endpoint with
...
do** â€” that is what a preview is for, and it is by far the fastest way to get it fixed.
...
at no cost â€” including commercial work you do as an individual.
...
[veloresearch.com](https://veloresearch.com) &nbsp;Â·&nbsp; [@velo_research](https://x.com/velo_research) &nbsp;Â·&nbsp; [contact@veloresearch.com](mailto:contact@veloresearch.com)
